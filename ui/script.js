...
function updateSpoonerHud(data) {
	...
	switch(data.entityType) {
		...
		case 3:
			...
			break;
			case 4:
				document.querySelector('#entity-type').innerHTML = 'Spooni';
				break;
		default:
...
function openSpawnMenu() {
	switch (lastSpawnMenu) {
		...
		case 4:
			...
			break;
		case 5:	
			document.querySelector('#spooni-menu').style.display = 'flex';
			break;
		default:
...
function closeSpooniMenu(selected) {
	document.querySelector('#spooni-menu').style.display = 'none';

	if (selected) {
		var name = selected.getAttribute('data-model');

		sendMessage('closeSpooniMenu', {
			modelName: name
		});

		document.querySelectorAll('#spooni-list .object').forEach(e => {
			if (favourites.spooni[e.getAttribute('data-model')]) {
				e.className = 'object favourite';
			} else {
				e.className = 'object';
			}
...
function updatePropertiesMenu(data) {
	...
	switch (properties.type) {
		...
		case 5:
			...
			break;
		case 6:
			document.querySelector('#properties-menu-entity-type').innerHTML = 'spooni';
			document.querySelectorAll('.spooni-property').forEach(e => e.style.display = 'block');
			break;	
		default:
...
function updatePermissions(data) {
	...
	document.getElementById('spawn-menu-spooni').disabled = !permissions.spawn.spooni;
...
window.addEventListener('load', function() {
	...
	document.querySelector('#spooni-spawn-by-name').addEventListener('click', function(event) {
		document.querySelector('#spooni-menu').style.display = 'none';

		sendMessage('closeSpooniMenu', {
			modelName: document.querySelector('#spooni-search-filter').value
		});
	});
	...
	document.querySelector('#spooni-menu-close-btn').addEventListener('click', function(event) {
		closeSpooniMenu();
	});
	...
	document.querySelector('#spawn-menu-spooni').addEventListener('click', function(event) {
		openSpooniMenu();
	});
	...
	document.querySelectorAll('.favourites').forEach(e => e.addEventListener('click', function(event) {
		...
		switch (this.id) {
			...
			case 'favourite-objects':
				...
				break;
			case 'favourite-spooni':
				populateSpooniList(document.getElementById('spooni-search-filter').value);
				break;
